...
export function buildFormFromConfig(config: unknown): CoreConfigForm {
  ...
  return {
    ...
    asrModelName: asString(voice.model_name),
    ...
    videoEnabled: asString(defaults.video_model) !== "",
    maxMediaSizeMB:
      typeof defaults.max_media_size === "number" &&
      Number.isFinite(defaults.max_media_size) &&
      defaults.max_media_size > 0
        ? String(Math.round(defaults.max_media_size / 1048576))
        : EMPTY_FORM.maxMediaSizeMB,
  }
}
