  ...
  const handleSave = async () => {
    try {
      ...
      if (configDirty) {
        ...
        const maxMediaSizeMB = parseIntField(
          form.maxMediaSizeMB,
          "Max media size",
          { min: 1 },
        )
        ...
        await patchAppConfig({
          agents: {
            defaults: {
              ...
              image_model: form.visionEnabled
                ? form.visionModelName.trim() || null
                : null,
              video_model: form.videoEnabled
                ? form.videoModelName.trim() || null
                : null,
              max_media_size: maxMediaSizeMB * 1048576,
            },
          },
          voice: {
